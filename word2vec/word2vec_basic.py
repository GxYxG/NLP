...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 11:36:22 2019
...
def generate_batch(batch_size, num_skips, skip_window):
  ...
  for i in range(batch_size // num_skips):
    ...
    # 如果到达数据尾部,重新开始，将数据前span存入buffer中，也就是说，是从数据第二个单词开始的
    if data_index == len(data):
      buffer.extend(data[0:span])
      data_index = span
    #如果没有越界，则在buffer尾部插入一个新单词，同时挤出buffer中第一个单词，相当于是span的范围向后移了一位
    else:
  ...
  with tf.name_scope('loss'):
    loss = tf.reduce_mean(
        tf.nn.nce_loss(
            ...
            num_classes=vocabulary_size))#50000
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    if step % 10000 == 0:
      sim = similarity.eval()
      for i in xrange(valid_size):#16个词
